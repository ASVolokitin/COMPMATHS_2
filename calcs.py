...
def newton(a, b, e, func, max_iter=500):
    ...
    for i in range(MAX_INTERVAL_LENGTH):
        ...
        x_new = x - func(x) / df
        n += 1
        if n > max_iter: return result_dict(None, None, "Превышено допустимое количество итераций.")
...
def simple_iteration(a, b, e, func, max_iterations = 1000):
    try:
        ...
        l = 1 / abs_der_max
        if derivative(func, (a + b) / 2) > 0: l *= -1
        phi = lambda x : x - l * func(x)
        if find_derivative_max(a, b, phi) >= 1:
            return result_dict(None, None, 0, "Достаточное условие сходимости не выполняется.")
        prev_arg = (a + b) / 2
